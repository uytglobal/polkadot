...
decl_storage! {
	trait Store for Module<T: Trait> as Attestations {
		...
		pub RecentParaBlocks: map hasher(twox_32_concat) T::BlockNumber => Option<IncludedBlocks<T>>;

		/// Attestations on a recent parachain block.
		pub ParaBlockAttestations:
			double_map hasher(twox_32_concat) T::BlockNumber, hasher(identity) Hash
