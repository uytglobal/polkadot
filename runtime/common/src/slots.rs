...
decl_storage! {
	trait Store for Module<T: Trait> as Slots {
		...
		pub Deposits get(fn deposits): map hasher(twox_32_concat) ParaId => Vec<BalanceOf<T>>;
		...
		pub Winning get(fn winning): map hasher(twox_32_concat) T::BlockNumber => Option<WinningData<T>>;

		/// Amounts currently reserved in the accounts of the bidders currently winning
		/// (sub-)ranges.
		pub ReservedAmounts get(fn reserved_amounts):
			map hasher(twox_32_concat) Bidder<T::AccountId> => Option<BalanceOf<T>>;
		...
		pub OnboardQueue get(fn onboard_queue): map hasher(twox_32_concat) LeasePeriodOf<T> => Vec<ParaId>;
		...
		pub Onboarding get(fn onboarding):
			map hasher(twox_32_concat) ParaId =>
		...
		pub Offboarding get(fn offboarding): map hasher(twox_32_concat) ParaId => T::AccountId;
	}
}
